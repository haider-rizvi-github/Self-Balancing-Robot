// MPU6050 integrated with LCD:  https://dronebotworkshop.com/mpu-6050-level/
...
// Define LCD I2C address - change if you have different address
...
int levelLED_pos0 = 13;

// Variables for gyroscope
int16_t gyro_x, gyro_y, gyro_z;           // To store gyro values
long gyro_x_cal, gyro_y_cal, gyro_z_cal;  // To store offset
...
void setup() {
  ...
  setup_mpu_6050_registers();  // This will go manually and setup the sensor registor
  ...
  Serial.begin(19200);  // Use it to trouble shoot (Note: Real projects don't need a serial monitor)
...
void setup_mpu_6050_registers() {
  ...
  Wire.write(0x00);  // setup to wake up the register
  Wire.endTransmission();

  // Configuring the accelerometer (+/- 8g)

  // Start communicating with MPU-6050
  Wire.beginTransmission(mpu6050_addr);
  Wire.write(0x1C);  // ACCEL_CONFIG register: 1C
  Wire.write(0x10);  // SELF_TEST_A register: 10
  Wire.endTransmission();
  ...
  // Start communicating with MPU6050
  Wire.beginTransmission(mpu6050_addr);
  Wire.write(0x1B);  // GYRO_CONFIG register; 1B
  Wire.write(0x08);  // NEED TO LOOK AT IT
  Wire.endTransmission();
};
